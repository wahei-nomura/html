...
<div class="wrap col-12" id="item-image">
	...
	<draggable v-model="商品画像" class="">
		<div v-for="(attr, index) in 商品画像" class="d-inline-block position-relative details attachment" style="width: auto;max-width: 25%;">
			...
			<!-- Download -->
			<a :href="`${ajaxurl}?action=n2_download_image_by_url&url=${attr.url}&name=${返礼品コード}${index > 0 ? '-'+index : ''}`" class="position-absolute bottom-0 end-0 badge rounded-pill bg-dark text-white text-decoration-none">▼ Download</a>
		</div>
	</draggable>

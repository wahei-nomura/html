...
if ( class_exists( 'N2_Rakuten_Import' ) ) {
	new N2_Rakuten_Import();
	return;
}
 ...
 * Rakuten_Import
 */
class N2_Rakuten_Import {
	/**
	 * コンストラクタ
	 */
	public function __construct() {
		add_action( 'wp_ajax_import_rakuten', array( $this, 'import_rakuten' ) );
	}
		/**
	 ...
	 * @return void
	 */
	public function import_rakuten(){
					...
					foreach ($tmp_name as $k => $file) {
						...
						if (strpos($name[$k], ".jpg") !== false) {
							...
							// キャビネットの場合
								...
								if ($m[1]) { //商品画像の場合
									if (ftp_mkdir($conn_id, $remote_dir)) echo "{$remote_dir}を作成\n";
									$remote_dir .= $m[1];
									if (ftp_mkdir($conn_id, $remote_dir)) echo "{$remote_dir}を作成\n";

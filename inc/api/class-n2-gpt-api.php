...
class N2_GPT_API {
	 ...
	 */
	public static function request_gpt( $usecase = '', $user_message = '' ) {
		global $n2;

		// もし引数がなかったらURLパラメータから変数生成
		$usecase      = empty( $usecase ) ? ( isset( $_GET['usecase'] ) ? $_GET['usecase'] : '' ) : $usecase;
		$user_message = empty( $user_message ) ? ( isset( $_GET['user_message'] ) ? $_GET['user_message'] : '' ) : $user_message;

		// gpt_template設定を取得
		$gpt_template   = $n2->gpt_template[ $usecase ];
		$model          = $gpt_template['model'] ?? 'gpt-3.5-turbo';
		$temperature    = $gpt_template['temperature'] ?? 0.5;
		$max_tokens     = $gpt_template['max_tokens'] ?? 1000;
		$system_message = $gpt_template['system_message'] ? array(
			'role'    => 'system',
			'content' => $gpt_template['system_message'],
		) : array();
		...
		// GPT APIへのリクエスト実行
		$response = wp_remote_request( static::$settings['endpoint'], $request );

		return json_decode( $response['body'] );
	}

	/**
	 * GPTのAPIキーをスプシから取得してセット
	 */
	...
	/**
	 * 秘密鍵を暗号化してsite_transientに保存する関数（なんかbase64_encodeよくわからんけど、お約束？）
	 *
	...
	/**
	 * site_transientから暗号化された値を取得し、復号化する関数（なんかbase64_encodeよくわからんけど、お約束？）
	 *
	 * @param string $key  transient key
	 ...
	 * 認証情報のキャッシュをリセット
	 */
	public static function reset_gpt_transient() {

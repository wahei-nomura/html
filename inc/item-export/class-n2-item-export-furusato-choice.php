...
class N2_Item_Export_Furusato_Choice extends N2_Item_Export_Base {
	...
	protected function walk_values( &$val, $index, $n2values ) {
		...
		$data = match ( 1 ) {
			...
			preg_match( '/地場産品類型番号/', $val )  => $n2values['地場産品類型'] ? ($n2values['地場産品類型'] === '2' || $n2values['地場産品類型'] === '3' || $n2values['地場産品類型'] === '6') ? "{$n2values['地場産品類型']}|{$n2values['類型該当理由']}" : "{$n2values['地場産品類型']}|" : '',// 設定したい地場産品類型番号と、地場産品に該当する理由（100文字以内）を入力してください。地場産品類型番号と該当する理由のテキストを『 | 』で区切ってください。

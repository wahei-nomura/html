    ...
    public function create_tsv() {
        ...
        foreach( $ids as $id ){
            ...
            $arr = array(
                ...
                '（必須）お礼の品名'       => N2_Functions::special_str_convert( get_the_title( $id ) ) . " [{$arr[$id]['管理コード']}]",
                // サイト表示事業者名：ポータル表示用名称が登録されてたら優先。「記載しない」になってたら空欄で出力。
                'サイト表示事業者名'      => get_the_author_meta( "portal", get_post_field( "post_author", $id ) ) ? ( get_the_author_meta( "portal", get_post_field( "post_author", $id ) ) === '記載しない' ? '' : get_the_author_meta("portal", get_post_field( "post_author", $id ) ) ) : get_the_author_meta( "first_name", get_post_field( "post_author", $id ) ), 
                'サイト表示事業者名' => get_post_meta($id, "提供事業者名", true) ?: $arr[$id]['サイト表示事業者名'], // NENGに提供事業者名が入ってたら優先
                '（条件付き必須）必要寄付金額'      => get_post_meta($id, "寄附金額", true),
                'キャッチコピー'  => N2_Functions::special_str_convert(get_post_meta($id, "キャッチコピー１", true)),
                '説明'     => N2_Functions::special_str_convert(get_post_meta($id, "説明文", true)) . (get_post_meta($id, "検索キーワード", true) ? "\n\n" . (N2_Functions::special_str_convert(get_post_meta($id, "検索キーワード", true))) : "") . (preg_match('/hasami/', esc_url($_SERVER['REQUEST_URI'])) ? (get_post_meta($id, "楽天カテゴリー", true) ? "\n\n" . (N2_Functions::special_str_convert(get_post_meta($id, "楽天カテゴリー", true))) : '') : ''),
                '容量'    => N2_Functions::special_str_convert(get_post_meta($id, "内容量・規格等", true)) . ((preg_match('/hasami|toki|ureshino|tajimi/', esc_url($_SERVER['REQUEST_URI'])) ? ((get_post_meta($id, "電子レンジ対応", true) != "-" || get_post_meta($id, "オーブン対応", true) != "-" || get_post_meta($id, "食洗機対応", true) != "-") ? "\n\n【対応機器】\n電子レンジ" . get_post_meta($id, "電子レンジ対応", true) . " / オーブン" . get_post_meta($id, "オーブン対応", true) . " / 食器洗浄機" . get_post_meta($id, "食洗機対応", true) . ((get_post_meta($id, "対応機器備考", true) != "") ? "\n※" . get_post_meta($id, "対応機器備考", true) : "") : "") : "") . ((preg_match('/hasami|toki|tajimi/', esc_url($_SERVER['REQUEST_URI'])) && get_post_meta($id, "やきもの", true) !== "やきものではない") ? "\n\n【お申し込み前に必ずお読みください】\nお届けする返礼品には個体差が生じる場合があります。\n下部記載の「個体差について」、「破損・欠陥について」を必ずご一読のうえ、お申し込みください。" . (preg_match('/toki|tajimi/', esc_url($_SERVER['REQUEST_URI'])) ? "\n\n【色について】\n商品写真はできる限り実物の色に近づけるよう徹底しておりますが、 お使いのモニター設定、お部屋の照明等により実際の商品と色味が異なる場合がございます。" : "") : "")) . (get_post_meta($id, "原料原産地", true) ? "\n\n【原料原産地】\n" . N2_Functions::special_str_convert(get_post_meta($id, "原料原産地", true)) : "") . (get_post_meta($id, "加工地", true) ? "\n\n【加工地】\n" . N2_Functions::special_str_convert(get_post_meta($id, "加工地", true)) : ""),
                '申込期日'      => get_post_meta($id, "申込期間", true),
                '発送期日'        => get_post_meta($id, "配送期間", true),
                '（必須）アレルギー表示'        => '',
                '地場産品類型番号'        => get_post_meta($id, "地場産品類型", true) ? get_post_meta($id, "地場産品類型", true) . "|" . get_post_meta($id, "類型該当理由", true) : "",
                '消費期限'        => (get_post_meta($id, "賞味期限", true) != "") ? "【賞味期限】\n" . N2_Functions::special_str_convert(get_post_meta($id, "賞味期限", true)) . ((get_post_meta($id, "消費期限", true) != "") ? "\n\n【消費期限】\n" . N2_Functions::special_str_convert(get_post_meta($id, "消費期限", true)) : "") : N2_Functions::special_str_convert(get_post_meta($id, "消費期限", true)),
                ...
                'カテゴリー'        => "",
                'お礼の品画像'        => mb_strtolower($arr[$id]['管理コード']) . ".jpg",

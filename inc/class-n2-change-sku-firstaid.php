...
class N2_Change_Sku_Firstaid {
	...
	public function output_csv( $csv_item_array, $csv_select_array ) {
		...
		foreach ( $csv_select_array as $subArray ) {
			$count = count( $subArray );
			if ( $count > $select_count ) {
				$select_count = $count;
			}
		}
		$select_count = $select_count - 2; // 返礼品コード、選択肢タイプ、商品オプション項目名分を除外(3ではなく2なのは保険)
		...
		for ( $i = 1; $i <= $select_count; $i++ ) {
			$new_item_column .= '商品オプション選択肢' . $i . ',';
		}
		$new_item_column .= '商品オプション選択必須,SKU管理番号,システム連携用SKU番号,販売価格,再入荷お知らせボタン,のし対応,在庫数,在庫あり時納期管理番号,送料,カタログIDなしの理由';

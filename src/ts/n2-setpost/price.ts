		...
		$.ajax({
			...
			class ControlSouryou {
				...
				public deliveryPattern: {
					[s: string]: { [s: string]: number | string };
				};

				constructor(
					webSyukka: boolean,
					deliveryCool: boolean,
					deliveryPattern: {
						[s: string]: { [s: string]: number | string };
					}
				) {
			...
			// 金額セット
			souryouState.setPrice = souryouState.convertPrice(
				$("#発送サイズ>option:selected").text()
			);
			...
			const changeSizeSelect = (souryouState) => {
				...
				$.each($("#発送サイズ>option"), (index, option) => {
					if (
						!Object.keys(
							souryouState.deliveryPattern["cool"]
						).includes($(option).text())
					) {
						$(option).css(
							"display",
							`${souryouState.deliveryCool ? "none" : "block"}`
						);
					}
				});
			};
			...
			const souryouDecision = (souryouState) => {
				...
				$("#送料").attr(
					"type",
					`${souryouState.webSyukka ? "hidden" : "text"}`
				);
			};
			...
			$("#発送方法").on("change", (e) => {
				...
				if (
					$("#発送サイズ>option:selected").text() !== "未選択" &&
					souryouState.deliveryCool
				) {
					alert(
						"発送方法が変更になったため発送サイズをリセットしました"
					);
			...
			const showPrice = (priceState): void => {
				if (
					Number(priceState.errorPrice()) >=
					Number($("#寄附金額").val())
				) {
					if (
						!$("#寄附金額").parent().find(`.${prefix}-alert`).length
					) {
						$("#寄附金額").before(
							$(
				...
				$("#寄附金額 + p").html(`
				自動計算の値：${priceState.calcPrice().toLocaleString()}（<span style="color:${
					priceState.diffPrice() >= 0 ? "turquoise" : "red"
				}">差額：${priceState.diffPrice().toLocaleString()}</span>）<br>
				価格：${(
					priceState.getkakaku * priceState.getteiki
				).toLocaleString()}（${priceState.getkakaku.toLocaleString()} × ${
					priceState.getteiki
				}回）<br>
				送料：${(
					priceState.getsouryou * priceState.getteiki
				).toLocaleString()}（${priceState.getsouryou.toLocaleString()} × ${
					priceState.getteiki
				}回）
				`);
			};
